...
app.post('/login/', async (request, response) => {
  ...
  if (dbUser === undefined) {
    ...
    const isPaswordMatched = await bcrypt.compare(password, dbUser.password)
    if (isPaswordMatched == true) {
      const payload = {
        username: username,
      }
...
app.get(
  ...
  async (request, response) => {
    const {stateId} = request.params
    const getDistrictStateQuery = `SELECT SUM(cases),SUM(cured),SUM(active),SUM(deaths) FROM district WHERE state_id=${stateId};`
    const stats = await db.get(getDistrictStateQuery)
    response.send({
